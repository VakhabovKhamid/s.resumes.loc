 ...
 * @var \App\Model\Entity\DictionaryIndustry[]|\Cake\Collection\CollectionInterface $dictionaryIndustries
 */
?>
<h2><?= __('Dictionary Industries') ?></h2>
<hr>
<div class="clearfix">
    <div class="pull-left">
...
<table cellpadding="0" cellspacing="0" class="table table-bordered table-striped table-condensed">
    ...
    <tbody>
        <?php foreach ($dictionaryIndustries as $dictionaryIndustry): ?>
        <tr>
            <td><?= $this->Number->format($dictionaryIndustry->id) ?></td>
            <td><?= h($dictionaryIndustry->name_uz_c) ?></td>
            <td><?= h($dictionaryIndustry->name_uz_l) ?></td>
            <td><?= h($dictionaryIndustry->name_ru_c) ?></td>
            <td><?= h($dictionaryIndustry->name_en_l) ?></td>
            <td><?= h($dictionaryIndustry->name_qr_c) ?></td>
            <td><?= h($dictionaryIndustry->name_qr_l) ?></td>
            <td><?= h($dictionaryIndustry->is_active) ?></td>
            <td><?= h($dictionaryIndustry->created) ?></td>
            <td><?= $this->Number->format($dictionaryIndustry->created_by) ?></td>
            <td><?= h($dictionaryIndustry->modified) ?></td>
            <td><?= $this->Number->format($dictionaryIndustry->modified_by) ?></td>
            <td class="actions">
                <?= $this->Html->link('<span class="fa fa-eye"></span>', ['action' => 'view', $dictionaryIndustry->id], ['escape' => false, 'class' => 'btn btn-default btn-sm']) ?>
                <?= $this->Html->link('<span class="fa fa-pencil"></span>', ['action' => 'edit', $dictionaryIndustry->id], ['escape' => false, 'class' => 'btn btn-default btn-sm']) ?>
                <?= $this->Form->postLink('<span class="fa fa-remove"></span>', ['action' => 'delete', $dictionaryIndustry->id], ['confirm' => __('Are you sure you want to delete # {0}?', $dictionaryIndustry->id), 'escape' => false, 'class' => 'btn btn-default btn-sm']) ?>
            </td>
        </tr>
        <?php endforeach; ?>
    </tbody>
